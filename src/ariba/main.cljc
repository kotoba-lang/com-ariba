(ns ariba.main
  "Kotodama WASM entrypoint for the Ariba clean-room actor (L4) — Clojure port.
  ...
  facts (`ariba.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"ariba.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ariba-compat" :tier tier :entities entities} 200])
